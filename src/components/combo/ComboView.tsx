...
/**
 * Main combo view component - significantly refactored for maintainability.
 * 
 * This component now uses custom hooks to manage separate concerns:
 * - useComboFilters: filter state and logic
 * - useComboSelection: multi-select state and operations
 * - useVideoPlayer: video player dialog state
 * - useComboDelete: delete confirmation and execution
 * - useComboOperations: combo CRUD operations
 *
 * Sub-components handle presentation:
 * - ComboViewHeader: character info display
 * - ComboViewToolbar: toolbar with filter, select, and add buttons
 * - ComboViewNotes: character notes section
 * - ComboSelectionToolbar: multi-select toolbar
 * - ComboSelectionToolbar: empty state display
 *
 * This reduces the component from 661 lines to ~300 lines, making it easier
	...
	return (
		<div>
			...
			<AlertDialog
				...
				onOpenChange={(open) =>
					!open && deleteState.setDeleteTarget(null)
				}
			>
				<AlertDialogContent>
					<AlertDialogHeader>
				...
				<AlertDialogContent>
					...
					<AlertDialogFooter>
						...
						<AlertDialogAction
							className="bg-destructive text-white hover:bg-destructive/90"
							onClick={async () => {
								await deleteState.executeBulkDelete(
